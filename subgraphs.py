...
def find_most_predictive_features(loaded_model, orig_data, reference):
    ...
    dataset = setup_dataset(input_data=orig_data, name="Calc. FP's", reference=reference, strip_ID=True)
...
if __name__ == "__main__":
    ...
    with open(os.path.join(output_dir, f'{apply_model}_on_{target_dataset}_sb_best_dict.pkl'), 'wb') as file:
        pickle.dump(best_dict, file)
    with open(os.path.join(output_dir, f'{apply_model}_on_{target_dataset}_sb_worst_dict.pkl'), 'wb') as file:
        pickle.dump(worst_dict, file)

    # print("WORST DICT:", worst_dict)
    # print("BEST DICT:", best_dict)

    for i, (ID, atomTuple) in enumerate(best_dict.items()):
        print("i, (ID, (atom_idx, tensor, degree)):", i, (ID, atomTuple))
